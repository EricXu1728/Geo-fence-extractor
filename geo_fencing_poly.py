...
def getFences(x, y, gdf):
    """
    Checks which geometries in the GeoDataFrame contain the point (x, y).
    Returns a list of names of those geometries.
    """
    point = gpd.points_from_xy([x], [y])[0]
    return gdf[gdf.geometry.contains(point)]["Name"].tolist()
...
def divideDataFrame(df, features, reset_sensitivity=2):
    """
    Divides a DataFrame into multiple DataFrames based on the zones defined in the GeoDataFrame features.
        ...
        for area in areas:
            if area in zone_last_seen:
                if index_counter - zone_last_seen[area] > reset_sensitivity:
                    ...
                    final_zone_dict[area].append(pd.DataFrame(zone_dict[area]))
                    zone_dict[area] = []
